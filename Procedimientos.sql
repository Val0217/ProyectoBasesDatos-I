    ...
    LOOP
        ...
        LOOP
            ...
            IF v_score >= v_min_score THEN
                ...
                INTO v_exists
                FROM PetMatch
                WHERE IdLostRepost = lost_rec.IdLostReport
...
BEGIN
    OPEN p_result FOR
        SELECT DISTINCT
            ...
            fh.NeedsDonation
        FROM FosterHome fh
        INNER JOIN Person p ON p.Id = fh.IdPerson
        ...
        WHERE (p_id_pet_size IS NULL OR psfh.IdPetSize = p_id_pet_size)
          AND (p_id_energy IS NULL OR pefh.IdEnergy = p_id_energy)
...
/*Insert person de Carlos por si acaso explota algo*/
...
/* ------------------------------------------------------------
   Procedimiento: pr_register_person
   Descripcion:
   Registra una persona en el sistema.
   ------------------------------------------------------------ */
CREATE OR REPLACE PROCEDURE pr_register_person (
...
/* ADOPTER */
CREATE OR REPLACE PROCEDURE pr_register_adopter (
    p_id_person IN NUMBER,
    p_new_id    OUT NUMBER
)
IS
BEGIN
    p_new_id := fn_next_id('Adopter');
...
/* RESCUER */
CREATE OR REPLACE PROCEDURE pr_register_rescuer (
    p_id_person IN NUMBER,
    p_new_id    OUT NUMBER
)
IS
BEGIN
    p_new_id := fn_next_id('Rescuer');
...
/* ADMIN */
CREATE OR REPLACE PROCEDURE pr_register_admin (
    p_id_person IN NUMBER,
    p_new_id    OUT NUMBER
)
IS
BEGIN
    p_new_id := fn_next_id('Admin');
...
CREATE OR REPLACE PROCEDURE pr_register_person_foster (
    p_needs_donation IN VARCHAR2,
    p_id_person      IN NUMBER,
    p_new_id         OUT NUMBER
)
...
/
/* ============================================================
   PROCEDIMIENTO COMPLETO PARA REGISTRAR PERSONA
   ============================================================ */


/* ------------------------------------------------------------
   Procedimiento: pr_register_full_person
   Descripcion:
   Registra una persona con correo, telefono y rol.
   ...
   Si no se envia rol, solo registra la persona.
   ------------------------------------------------------------ */
CREATE OR REPLACE PROCEDURE pr_register_full_person (
    p_first_name     IN VARCHAR2,
    p_last_name      IN VARCHAR2,
    p_password       IN VARCHAR2,
    p_id_district    IN NUMBER,
    ...
    p_needs_donation IN VARCHAR2 DEFAULT NULL,
    p_new_person_id  OUT NUMBER
)
IS
    v_email_id NUMBER;
    v_phone_id NUMBER;
...
BEGIN
    pr_register_person(
        ...
        p_id_district => p_id_district,
        p_new_id      => p_new_person_id
    );

    IF p_email IS NOT NULL THEN
        pr_register_person_email(
            p_email     => p_email,
            p_id_person => p_new_person_id,
            p_new_id    => v_email_id
        );
    END IF;

    IF p_phone IS NOT NULL THEN
        pr_register_person_phone(
            p_phone     => p_phone,
            p_id_person => p_new_person_id,
            p_new_id    => v_phone_id
        );
    ...
    IF v_role = 'ADOPTER' THEN
        pr_register_adopter(p_new_person_id, v_role_id);

    ELSIF v_role = 'RESCUER' THEN
        pr_register_rescuer(p_new_person_id, v_role_id);

    ELSIF v_role = 'ADMIN' THEN
        pr_register_admin(p_new_person_id, v_role_id);

    ELSIF v_role = 'FOSTER' THEN
        pr_register_person_foster(
            p_needs_donation => NVL(p_needs_donation, 'N'),
            p_id_person      => p_new_person_id,
            p_new_id         => v_role_id
        );
    END IF;
